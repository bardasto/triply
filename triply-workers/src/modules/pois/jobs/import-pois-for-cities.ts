...
import getSupabaseAdmin, { batchInsert } from '../../../shared/config/supabase.js';
import googlePlacesService from '../../google-places/services/google-places.service.js';
import logger from '../../../shared/utils/logger.js';
import config from '../../../shared/config/env.js';
import type { City, POI, POIInput, POICategory, ActivityType } from '../../../shared/types/index.js';
...
const PLACE_TYPE_TO_CATEGORY: Record<string, POICategory> = {
  ...
  courthouse: 'landmark',
  embassy: 'landmark',
  local_government_office: 'landmark',
  ...
  hair_care: 'entertainment',

  // Transit - can be landmarks for city tours
  airport: 'landmark',
...
async function fetchPOIsForCity(city: City, maxPois: number): Promise<POIInput[]> {
  ...
  // ✅ MASSIVE list (~100 types) to support ALL trip types and ensure diversity
  const searchTypes = [
    // Main attractions
    'tourist_attraction',
    'point_of_interest',
...
function mapPlaceToPOI(place: any, cityId: string): POIInput | null {
  ...
  return {
    ...
    metadata: {
      // ⚠️ NOTE: nearbySearch may not return full opening_hours with weekday_text
      // Consider enriching with getPlaceDetails() later for complete data
